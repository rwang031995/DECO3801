...
const ChallengesScreen = ({navigation}) => {
  ...
  const updateWeeklyReset = () => {
    let currentWeek = moment().startOf('isoWeek').add(1, 'days');
    let previousWeek = moment(storedWeek);
    if (currentWeek.clone().subtract(7, 'days').isSameOrAfter(previousWeek)) {
      let newWeek = JSON.stringify(currentWeek).substring(1, JSON.stringify(currentWeek).length - 1);
      let challengesComplete = true;
